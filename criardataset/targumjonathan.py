    ...
    for book in ot_books:
        ...
        while True:
            ...
            if "next" in data and data["next"] is not None:
                ...
                match = re.search(r'(Targum Jonathan on )([A-Za-z ]+)( [0-9]+)', text)
                ...
                if match:
                    ...
                    print(book)
                    print(chapter)
                    url = f"https://www.sefaria.org/api/texts/{rabino}_on_{book}.{str(chapter).strip()}?commentary=0&context=1&pad=0&wrapLinks=1&wrapNamedEntities=1&multiple=0&stripItags=0&transLangPref=&firstAvailableRef=1&fallbackOnDefaultVersion=1"
                    print(url)
                else:
