...
for rabino in rabinos:
    ...
    for book in ot_books:
        ...
        url = f"https://www.sefaria.org/api/texts/{rabino}_{book}.1?commentary=0&context=1&pad=0&wrapLinks=1&wrapNamedEntities=1&multiple=0&stripItags=0&transLangPref=&firstAvailableRef=1&fallbackOnDefaultVersion=1"
        print(url)
        while True:
            ...
            if "text" in data:
                ...
                with open(rabino+'_ot.csv', 'a', newline='', encoding='utf-8') as file:
                    ...
                    for text in data["text"]:
                        ...
                        writer.writerow([book.strip(), str(chapter).strip(), str(verse).strip(), text])
                        verse += 1
            ...
            if "next" in data and data["next"] is not None:
                ...
                match = re.search(r'('+rabino+' )([A-Za-z ]+)( [0-9]+)', text)
                if match:
                    book = match.group(2)
                    ...
                    print(book)
                    print(chapter)
                    url = f"https://www.sefaria.org/api/texts/{rabino}_{book}.{str(chapter).strip()}?commentary=0&context=1&pad=0&wrapLinks=1&wrapNamedEntities=1&multiple=0&stripItags=0&transLangPref=&firstAvailableRef=1&fallbackOnDefaultVersion=1"
                    print(url)
                else:
